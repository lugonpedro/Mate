...
export default class Map extends React.Component {
    ...
    async userExists() {
        const userRef = await firebase.firestore().collection("motorista").doc(firebase.auth().currentUser.uid).get().then(doc => {
            if (doc.exists) {
                if (doc.data().latitudeS != null &&
                    doc.data().longitudeS != null &&
                    doc.data().latitudeC != null &&
                    doc.data().longitudeC != null) {
                    firebase.firestore().collection("motorista").doc(firebase.auth().currentUser.uid).onSnapshot(doc => {
                        this.setState({
                            destinationStay: {
