  ...
  function register() {
    try {
      if (email.length < 10 && senha.length < 6) {
        Alert.alert(
          "Por favor, digite um e-mail valido e uma senha com mais de 6 caracteres"
        );
      } else {
        firebase
          .auth()
          ...
          .then((resultado) => {
            ...
            Alert.alert(
              "Cadastro realizado",
              "Por favor, confirme sua conta pelo e-mail"
            );
          });
      }
    } catch (error) {
      ...
      {seeing || (
        <Fragment>
          <TouchableOpacity
            style={styles.olho}
            onPress={() => {
              setSeeing(true) & setKeyboard("visible-password");
            }}
          >
      ...
      {seeing && (
        <Fragment>
          <TouchableOpacity
            style={styles.olho}
            onPress={() => {
              setSeeing(false) & setKeyboard("default");
            }}
          >
